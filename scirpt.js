...
//Get top,bottom,left,right in function
...
//input-cell ke selection and de-select pr jo kaam kiya he  
...
//Changing the header while chnaging the input-cell alignment

function changeHeader([rowId, colId]) {
    let data = cellData[rowId-1][colId-1];
    $(".alignment.selected").removeClass("selected");
    $(`.alignment[data-type=${data.alignment}]`).addClass("selected");
    //DOUBT SIR (Unselect nhi hote)
...
$(".input-cell").mousemove(function(e) {
    ...
    if (e.buttons == 1) {
        ...
        if (!startCellSelected) {
            ...
            selectBetweenCells(startCell, startCell);
...
$(".input-cell").mouseenter(function(e) {
    if(e.buttons == 1) {
        ...
        selectBetweenCells(startCell, endCell);
    }
});

// select between input-cell
function selectBetweenCells(start, end) {
    $(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected");
    for (let i = Math.min(start.rowId, end.rowId); i <= Math.max(start.rowId, end.rowId); i++) {
        for (let j = Math.min(start.colId, end.colId); j <= Math.max(start.colId, end.colId); j++) {
            ...
            selectCell($(`#row-${i}-col-${j}`)[0], { "metaKey": true }, topCell, bottomCell, leftCell, rightCell);
        }
    }
}
...
//Changing the alignment of input cell
...
//Setting bold, italic and underlined style to text
